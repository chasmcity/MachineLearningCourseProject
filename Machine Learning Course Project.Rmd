...
fit
```
fit$finalModel gives a confusion matrix with error per class. The confusion matrix suggests the model is most accurate at predicting class A which users fall into class A, although the error is <0.004 for all classes.
```{r}
fit$finalModel
```
